...
export default async function AnalyticsPage({
  ...
  const periodLabel =
    periods.find((item) => item.value === period)?.label ?? "Últimos 7 dias";
  const data = await getAnalytics(range);
  const maxDaily = Math.max(
    ...data.daily.map((day) => Math.max(day.visitors, day.whatsappClicks)),
    1,
  );
  const hasData =
    data.summary.visitors > 0 ||
    data.summary.pageViews > 0 ||
    data.summary.diagnosticStarted > 0 ||
    data.summary.whatsappClicks > 0;
      ...
      <div className="mx-auto max-w-[1200px]">
        <div className="flex flex-col gap-5 border-b border-primary/15 pb-6 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <p className="mt-3 max-w-2xl text-sm text-primary/70">
              Aquisição e conversão da landing page
            </p>
            ...
            <nav className="flex flex-wrap gap-2" aria-label="Período">
              {periods.map((item) => (
                <a
                  ...
                  className={`rounded-md border px-3 py-2 text-sm ${period === item.value ? "border-primary bg-primary text-primary-foreground" : "border-primary/15 text-primary"}`}
                >
                  {item.label}
                </a>
              ))}
        ...
        <section className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <Metric
            ...
            value={data.summary.diagnosticStarted}
          />
          <Metric
            label="Diagnósticos concluídos"
            value={data.summary.diagnosticCompleted}
          />
          <Metric
          ...
          <Metric
            label="Início / page view"
            value={formatPercent(data.summary.diagnosticStartRate)}
          />
          <Metric
            label="Conclusão / início"
            value={formatPercent(data.summary.diagnosticCompletionRate)}
          />
          <Metric
            label="WhatsApp / conclusão"
            value={formatPercent(data.summary.whatsappAfterDiagnosticRate)}
          />
        </section>
        ...
        <section className="mt-10">
          ...
          <div className="mt-4 overflow-x-auto border-y border-primary/15">
            <table className="w-full min-w-[560px] text-left text-sm">
              <thead className="text-xs uppercase tracking-[0.16em] text-primary/55">
                <tr>
                  <th className="py-3">Data</th>
                  <th>Visitantes</th>
                  <th>WhatsApp</th>
                  <th className="w-1/3">Volume</th>
                </tr>
              </thead>
              <tbody>
                {data.daily.map((day) => (
                  <tr key={day.date} className="border-t border-primary/10">
                    <td className="py-3">{formatShortDate(day.date)}</td>
                    <td>{day.visitors}</td>
                    <td>{day.whatsappClicks}</td>
                    <td>
                      <Bar value={day.visitors} max={maxDaily} />
                      <Bar value={day.whatsappClicks} max={maxDaily} accent />
                    </td>
                  </tr>
                ))}
                {!data.daily.length && (
        ...
        <Table
          ...
          rows={data.sources}
          firstColumn="Origem"
          emptyText="Nenhuma origem identificada neste período."
        />
        <Table
          title="Campanhas"
          rows={data.campaigns}
          firstColumn="Campanha"
          emptyText="Nenhuma campanha identificada neste período."
        />
        <Table
          title="Conteúdos"
          rows={data.contents}
          firstColumn="Conteúdo"
          emptyText="Nenhum utm_content identificado neste período."
        />
        ...
        <section className="mt-10">
          ...
          <div className="mt-4 grid gap-3">
            {data.insights.map((insight) => (
              <p
                key={insight.text}
                className={`border-l-2 px-4 py-3 text-sm ${insight.warning ? "border-accent bg-accent/10" : "border-primary/25 bg-card"}`}
              >
                {insight.text}
              </p>
            ))}
        ...
        <p className="mt-8 text-xs text-primary/50">
...
function Metric({ label, value }: { label: string; value: number | string }) {
  return (
    <div className="rounded-md border border-primary/15 bg-card p-4">
      <p className="text-xs uppercase tracking-[0.16em] text-primary/55">
        {label}
      </p>
    ...
    <section className="mt-10">
      ...
      <div className="mt-4 overflow-x-auto border-y border-primary/15">
        <table className="w-full min-w-[520px] text-left text-sm">
          <thead className="text-xs uppercase tracking-[0.16em] text-primary/55">
            <tr>
              <th className="py-3">{firstColumn}</th>
              <th>Visitantes</th>
              <th>Cliques</th>
              <th>Conversão</th>
            </tr>
...
function Bar({
  ...
  return (
    <span
      className={`mb-1 block h-2 rounded-sm ${accent ? "bg-accent" : "bg-primary"}`}
  ...
  return (
    <section className="mt-6 border border-primary/15 bg-card p-5">
      ...
      <p className="mt-2 text-sm text-primary/65">
        Abra a landing page por um link com UTM e clique no WhatsApp para
        validar o tracking.
      </p>
    </section>
  );
